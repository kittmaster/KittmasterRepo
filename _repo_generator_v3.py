...
def _setup_colors():
    ...
    return any([
        is_a_tty(), sys.platform != "win32", "ANSICON" in os.environ, "WT_SESSION" in os.environ,
        os.environ.get("TERM_PROGRAM") == "vscode", vt_codes_enabled_in_windows_registry(), legacy_support(),
    ])
...
class Generator:
    ...
    def _create_zip(self, folder, addon_id, version):
        ...
        if os.path.exists(f"{final_zip_path}.md5"):
            os.remove(f"{final_zip_path}.md5")
        ...
        try:
            ...
            zip_hash = self._calculate_file_hash(final_zip_path)
    ...
    def _generate_addons_file(self, addons_xml_path):
        """ Generates zips for new/updated addons and regenerates the master addons.xml. """
        old_addons = {}
        if os.path.exists(addons_xml_path):
            try:
                for addon in ElementTree.parse(addons_xml_path).getroot().findall("addon"):
                    old_addons[addon.get("id")] = addon.get("version")
            except ElementTree.ParseError as e:
                print(f"Warning: Could not parse {addons_xml_path}. Rebuilding all. Error: {e}")

        folders = [
            i for i in os.listdir(self.release_path) if os.path.isdir(os.path.join(self.release_path, i))
            and i != "zips"
        ...
        for addon_name in folders:
            try:
                addon_root = ElementTree.parse(os.path.join(self.release_path, addon_name, "addon.xml")).getroot()
                addon_id, addon_version = addon_root.get('id'), addon_root.get('version')

                if self.build_mode == 'all' or old_addons.get(addon_id) != addon_version:
                    if self.build_mode == 'changed':
                        print(f"Found update for: {color_text(addon_id, 'cyan')} ({color_text(old_addons.get(addon_id, 'new'), 'yellow')} -> {color_text(addon_version, 'green')})")
                    has_changes = True
        ...
        if self.build_mode == 'all' or has_changes:
            ...
            try:
                ElementTree.ElementTree(new_addons_root).write(addons_xml_path, encoding="utf-8", xml_declaration=True)
                return True
            except Exception as e:
...
if __name__ == "__main__":
    ...
    while build_mode not in ('all', 'changed'):
        ...
        if response in ('all', 'a'):
            build_mode, msg = 'all', "Building all zips (existing zips will be overwritten)."
        elif response in ('changed', 'c'):
            build_mode, msg = 'changed', "Building only new or updated zips."
        else:
