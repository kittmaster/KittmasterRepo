...
SCRIPT_VERSION = 5
# Updated KODI_VERSIONS to include "scripts-addons"
KODI_VERSIONS = ["krypton", "leia", "matrix", "nexus", "repo", "skin", "omega", "piers", "scripts-addons"]
IGNORE = [
    ".git",
    ".github",
    ".gitignore",
...
class Generator:
    ...
    def _create_zip(self, folder, addon_id, version):
        ...
        Based on the build_mode, it either always rebuilds or only if content changes.
        """
        ...
        addon_xml_source_path = os.path.join(addon_folder, "addon.xml")
        zip_content_hash_file = os.path.join(zip_folder, f"{addon_id}-{version}.hash")

        rebuild_zip = False

        if self.build_mode == 'all':
            if os.path.exists(final_zip):
                print(f"[{color_text(addon_id, 'cyan')}] {color_text('Deleting existing zip (Build All mode).', 'blue')}")
                os.remove(final_zip)
                # Also remove the hash file so a new one is generated
                if os.path.exists(zip_content_hash_file):
                    os.remove(zip_content_hash_file)
            rebuild_zip = True
        elif self.build_mode == 'changed':
            if not os.path.exists(final_zip):
                print(f"[{color_text(addon_id, 'cyan')}] {color_text('Zip does not exist, building.', 'blue')}")
                rebuild_zip = True
            else:
                current_addon_xml_hash = self._calculate_file_hash(addon_xml_source_path)
                stored_zip_content_hash = None
                if os.path.exists(zip_content_hash_file):
                    with open(zip_content_hash_file, 'r') as f:
                        stored_zip_content_hash = f.read().strip()

                if current_addon_xml_hash != stored_zip_content_hash:
                    print(f"[{color_text(addon_id, 'cyan')}] {color_text('addon.xml content changed or hash missing, rebuilding zip.', 'blue')}")
                    if os.path.exists(final_zip):
                        os.remove(final_zip)
                    rebuild_zip = True
                else:
                    print(f"[{color_text(addon_id, 'cyan')}] {color_text('addon.xml content unchanged, skipping zip rebuild.', 'blue')}")
                    rebuild_zip = False
        ...
        if rebuild_zip:
            ...
            for root, dirs, files in os.walk(addon_folder):
                ...
                if archive_root.startswith(os.sep):
                    archive_root = archive_root[len(os.sep):]
            ...
            if self.build_mode == 'changed':
                with open(zip_content_hash_file, 'w') as f:
                    f.write(current_addon_xml_hash)
        else:
            print(
                "Zip already exists for {} ({}) - {}".format(
                    color_text(addon_id, 'cyan'),
                    color_text(version, 'green'),
                    color_text("skipped", 'blue'),
                )
            )
    ...
    def _generate_addons_file(self, addons_xml_path):
        ...
        for addon_name in folders:
            try:
                addon_xml_path_in_folder = os.path.join(self.release_path, addon_name, "addon.xml")
                addon_xml_element = ElementTree.parse(addon_xml_path_in_folder)
                addon_root_element = addon_xml_element.getroot()
                addon_id = addon_root_element.get('id')
                addon_version = addon_root_element.get('version')

                updated_repo_xml = False
                addon_entry = addons_root.find(addon_xpath.format(addon_id))
                if addon_entry is not None and addon_entry.get('version') != addon_version:
                    index = addons_root.findall('addon').index(addon_entry)
                    addons_root.remove(addon_entry)
                    addons_root.insert(index, addon_root_element)
                    updated_repo_xml = True
                    changed = True
                elif addon_entry is None:
                    addons_root.append(addon_root_element)
                    updated_repo_xml = True
                    changed = True
                ...
                print(
                    "Excluding {}: {}".format(
                        color_text(addon_name, 'yellow'), color_text(str(e), 'red')
                    )
                )
            ...
            try:
                ...
                print(
                    "An error occurred updating {}:\n{}".format(
                        color_text(addons_xml_path, 'yellow'), color_text(str(e), 'red')
                    )
                )
        return False
    ...
    def _generate_md5_file(self, addons_xml_path, md5_path):
        ...
        try:
            ...
            print(
                "An error occurred updating {}:\n{}".format(
                    color_text(md5_path, 'yellow'), color_text(str(e), 'red')
                )
            )
            return False
        ...
        try:
            ...
            print(
                "An error occurred saving {}:\n{}".format(
                    color_text(file, 'yellow'), color_text(str(e), 'red')
                )
            )
...
if __name__ == "__main__":
    ...
    while build_mode not in ('all', 'changed'):
        ...
        if response in ('all', 'a'):
            ...
            build_mode = 'changed'
            print(color_text("Building only zips where addon.xml content or version has changed.", 'blue'))
        else:
            print(color_text("Invalid input. Please enter 'all', 'changed', 'a', or 'c'.", 'red'))
